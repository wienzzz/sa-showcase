...
export const findBetweenDates = async (req: Request, h: ResponseToolkit) => {
  logger.info("Find shifts between dates");
  try {
    const filter = req.query;
    const data = await shiftUsecase.findBetweenDates(filter);
    const res: ISuccessResponse = {
      statusCode: 200,
      message: "Get shift successful",
      results: data,
